...
function handleSubscribe(ws, json) {
  ...
  if (!topic) return { status: "cannot fetch topic", type: "subscribe", success: false };
  ...
  if (joined) rt["data"] = topic.getData();
  return rt;
}

function handleNotify(ws, json) {
  const topic = _messageSerivice2.default.getTopic(json.topic);
  if (!topic) return { status: "cannot fetch topic", type: "notify", success: false };
